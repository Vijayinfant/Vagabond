...
export default function TripDetails() {
  ...
  if (authLoading || !user) {
    return (
      <div className="flex-1 flex flex-col justify-center items-center min-h-screen theme-trip-details">
  ...
  if (loadingTrip) {
    return (
      <div className="flex-1 flex flex-col justify-center items-center min-h-screen theme-trip-details">
        <div className="w-8 h-8 border-4 border-cyan-500 border-t-transparent rounded-full animate-spin" />
        <p className="mt-4 text-slate-500 text-sm font-medium">Fetching your travel plan details...</p>
      </div>
    );
  }
  ...
  if (error || !trip) {
    return (
      <div className="flex-1 flex flex-col justify-center items-center min-h-screen px-6 theme-trip-details">
        <div className="glass-panel p-8 rounded-2xl max-w-md text-center border border-rose-200/50">
          <AlertTriangle className="w-12 h-12 text-rose-500 mx-auto mb-4" />
          <h2 className="text-xl font-bold text-slate-900 mb-2">Error loading trip</h2>
          <p className="text-slate-600 text-sm mb-6">{error || 'The requested trip itinerary does not exist.'}</p>
          <Link href="/dashboard" className="px-5 py-2.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-800 text-sm font-semibold border border-slate-200 transition-all">
            Return to Dashboard
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="flex-grow flex flex-col min-h-screen theme-trip-details relative">
      ...
      <header className="relative z-10 bg-white/40 backdrop-blur-md border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <Link href="/dashboard" className="flex items-center gap-1.5 text-slate-500 hover:text-slate-900 text-sm font-semibold transition-colors">
      ...
      <main className="relative z-10 flex-grow max-w-7xl mx-auto w-full px-6 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="glass-panel p-6 rounded-2xl border border-slate-200/60 shadow-xl relative overflow-hidden">
            <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-cyan-500 to-blue-600" />
            
            <h1 className="text-2xl font-black text-slate-900 mb-4 mt-2">{trip.destination}</h1>
            ...
            <div className="space-y-3.5 text-sm text-slate-600">
              ...
              <div className="flex items-center gap-2.5">
                <Clock className="w-4.5 h-4.5 text-cyan-600 shrink-0" />
                <span>{trip.durationDays} Days Duration</span>
              </div>
              <div className="flex items-center gap-2.5">
                <DollarSign className="w-4.5 h-4.5 text-cyan-600 shrink-0" />
                <span>Budget Tier: <span className="font-semibold text-slate-900">{trip.budgetPreference}</span></span>
              </div>
            </div>
            ...
            {trip.interests && trip.interests.length > 0 && (
              <div className="mt-6 pt-5 border-t border-slate-200">
                <p className="text-slate-500 text-xs font-semibold uppercase tracking-wider mb-2.5">Focus Interests</p>
                <div className="flex flex-wrap gap-1.5">
                  {trip.interests.map((interest) => (
                    <span 
                      key={interest} 
                      className="px-2.5 py-1 rounded-md bg-slate-100 border border-slate-200/60 text-slate-600 text-xs font-medium"
                    >
                      {interest}
                    </span>
                  ))}
          ...
          <div className="glass-panel p-6 rounded-2xl border border-slate-200/60 shadow-xl space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-slate-500 text-sm uppercase tracking-wider">Budget Progress</h3>
              {totalActual > totalEstimated ? (
                <span className="flex items-center gap-1 text-rose-600 text-xs font-bold bg-rose-500/10 px-2 py-0.5 rounded">
                  <AlertTriangle className="w-3.5 h-3.5" />
                  Over Budget
                </span>
              ) : (
                <span className="flex items-center gap-1 text-emerald-600 text-xs font-bold bg-emerald-500/10 px-2 py-0.5 rounded">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-xs text-slate-500">
                ...
                <span className="text-slate-800 font-semibold">${totalEstimated}</span>
              </div>
              <div className="flex justify-between text-xs text-slate-500">
                <span>Actual Logged Spend:</span>
                <span className={`font-bold ${totalActual > totalEstimated ? 'text-rose-600' : 'text-emerald-600'}`}>
                  ${totalActual}
                </span>
              </div>

              {/* Progress Bar SVG/CSS */}
              <div className="w-full bg-slate-200/60 rounded-full h-2.5 mt-4 overflow-hidden border border-slate-200/60">
                <div 
                  className={`h-full rounded-full transition-all duration-500 ${
                    totalActual > totalEstimated ? 'bg-rose-500' : 'bg-gradient-to-r from-cyan-500 to-emerald-500'
                  }`}
              ...
              {totalActual > totalEstimated && (
                <p className="text-rose-600 text-xs font-medium leading-relaxed mt-2.5">
        ...
        <div className="lg:col-span-8 flex flex-col space-y-6">
          ...
          <div className="flex rounded-xl bg-slate-100 p-1 border border-slate-200/60 self-start">
            <button
          ...
          {activeTab === 'itinerary' && (
            <div className="space-y-6">
                  ...
                  <button
                    ...
                    className={`flex-shrink-0 px-4 py-2.5 rounded-xl text-sm font-bold border transition-all ${
                      activeDay === day.dayNumber
                        ? 'bg-cyan-500/10 border-cyan-500 text-cyan-600 font-semibold'
                        : 'bg-white/40 border-slate-200/60 text-slate-500 hover:text-slate-900 hover:border-slate-300'
                    }`}
                  >
                    Day {day.dayNumber}
                  </button>
              ...
              {/* Day panel */}
              <div className="glass-panel p-6 sm:p-8 rounded-2xl border border-slate-200/60 shadow-xl space-y-6">
                <div className="flex justify-between items-center pb-4 border-b border-slate-200/60">
                  <div>
                    <h2 className="text-xl font-bold text-slate-900">Day {activeDay} Schedule</h2>
                    <p className="text-xs text-slate-500 mt-1">Explore carefully mapped spots</p>
                  </div>
                  <button
                    onClick={() => setShowAddActivityForm(!showAddActivityForm)}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-100 border border-slate-200 text-xs text-cyan-600 hover:text-cyan-700 hover:bg-slate-200 font-bold transition-all"
                  >
                ...
                {showAddActivityForm && (
                  <form onSubmit={handleAddActivity} className="p-4 rounded-xl bg-white border border-slate-200 shadow-sm space-y-3">
                    <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">Add Activity Manually</p>
                    ...
                    <button
                      ...
                      className="px-4 py-2 rounded-lg bg-cyan-650 hover:bg-cyan-550 text-white font-bold text-xs transition-all disabled:opacity-50"
                    >
                  ...
                  <div className="relative pl-6 border-l border-slate-200/60 space-y-8 py-2">
                    {activeDayPlan?.activities.map((activity) => (
                      <div key={activity.id} className="relative group/item">
                        {/* Timeline bubble */}
                        <div className="absolute left-[-30px] top-1.5 w-4.5 h-4.5 rounded-full bg-white border-2 border-cyan-500 flex items-center justify-center shrink-0 animate-pulse" />
                        
                        <div className="glass-card p-5 rounded-xl flex flex-col sm:flex-row justify-between items-start gap-4">
                          <div className="space-y-1">
                            <span className="inline-block px-2.5 py-0.5 rounded bg-cyan-500/10 border border-cyan-500/20 text-cyan-600 font-bold text-[10px]">
                              {activity.time}
                            </span>
                            <h4 className="text-base font-bold text-slate-900 pt-1">{activity.title}</h4>
                            <p className="text-sm text-slate-650 leading-relaxed pt-1.5">{activity.description}</p>
                          </div>
                          
                          <button
                            onClick={() => handleDeleteActivity(activeDay, activity.id)}
                            className="p-1.5 rounded bg-slate-100 hover:bg-rose-500/10 border border-slate-200 text-slate-500 hover:text-rose-600 transition-all sm:opacity-0 group-hover/item:opacity-100 focus:opacity-100 shrink-0 self-end sm:self-start"
                ...
                <div className="mt-8 pt-6 border-t border-slate-200 space-y-4">
                  ...
                  <div className="flex gap-2.5">
                    ...
                    <button
                      ...
                      className="px-4 py-2.5 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white font-bold text-xs transition-all flex items-center gap-1.5 disabled:opacity-50"
          ...
          {activeTab === 'hotels' && (
            <div className="space-y-6">
              <div className="glass-panel p-6 rounded-2xl border border-slate-200/60 shadow-xl">
                <div className="pb-4 border-b border-slate-200/60 mb-6">
                  <h2 className="text-xl font-bold text-slate-900">Recommended Places to Stay</h2>
                  <p className="text-xs text-slate-500 mt-1">Suggested hotels matched to budgets and ratings</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {trip.hotels.map((hotel) => {
                    const priceColors = {
                      'Budget Friendly': 'text-emerald-700 bg-emerald-500/10 border-emerald-500/20',
                      'Mid Range': 'text-cyan-700 bg-cyan-500/10 border-cyan-500/20',
                      'Luxury': 'text-purple-700 bg-purple-500/10 border-purple-500/20'
                    };
                    const colorClass = priceColors[hotel.priceTier] || 'text-slate-600 bg-slate-100 border-slate-200';
          ...
          {activeTab === 'expenses' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                {/* Form to log expense */}
                <div className="md:col-span-5 glass-panel p-5 rounded-2xl border border-slate-200/60 shadow-xl">
                  <h3 className="font-bold text-slate-850 text-sm uppercase tracking-wider mb-4 text-cyan-600">Log Travel Expense</h3>
                  
                  <form onSubmit={handleAddExpense} className="space-y-3.5">
                    <div>
                      <label className="block text-slate-600 text-[10px] font-semibold uppercase tracking-wider mb-1.5">Category</label>
                      <select
                        className="glass-input w-full p-2.5 rounded-lg text-xs bg-white"
                        value={expCategory}
                    ...
                    <div>
                      <label className="block text-slate-600 text-[10px] font-semibold uppercase tracking-wider mb-1.5">Amount (USD)</label>
                      <div className="relative">
                        ...
                        <input
                          ...
                          min="0.01"
                          placeholder="24.99"
                          className="glass-input w-full pl-6 pr-3 py-2 rounded-lg text-xs bg-white"
                          value={expAmount}
                    ...
                    <div>
                      <label className="block text-slate-600 text-[10px] font-semibold uppercase tracking-wider mb-1.5">Description</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="block text-slate-600 text-[10px] font-semibold uppercase tracking-wider mb-1.5">Date</label>
                      <input
                        type="date"
                        className="glass-input w-full p-2.5 rounded-lg text-xs bg-white [color-scheme:light]"
                        value={expDate}
                    ...
                    <button
                      type="submit"
                      disabled={loggingExpense}
                      className="w-full py-2.5 rounded-lg bg-cyan-600 hover:bg-cyan-550 text-white font-bold text-xs transition-all flex items-center justify-center gap-1.5 disabled:opacity-50 mt-4 cursor-pointer"
                    >
                      {loggingExpense ? (
                        <div className="w-3.5 h-3.5 border border-white border-t-transparent rounded-full animate-spin" />
                      ) : (
                ...
                <div className="md:col-span-7 glass-panel p-5 rounded-2xl border border-slate-200/60 shadow-xl flex flex-col justify-between">
                  <div>
                    <h3 className="font-bold text-slate-800 text-sm uppercase tracking-wider mb-3.5 text-cyan-600">
                        ...
                        return (
                          <div key={est.category} className="space-y-1.5">
                            <div className="flex justify-between text-xs font-medium">
                              <span className="text-slate-700 font-semibold">{est.category}</span>
                            ...
                            <div className="w-full h-2 rounded-full bg-slate-200 overflow-hidden border border-slate-200/60">
                              <div 
                                className={`h-full rounded-full transition-all duration-300 ${
                                  isOver ? 'bg-rose-500 animate-pulse' : 'bg-cyan-500'
                                }`}
                                style={{ width: `${pct}%` }}
                              />
                            </div>
              ...
              <div className="glass-panel p-6 rounded-2xl border border-slate-200/60 shadow-xl">
                <h3 className="font-bold text-slate-850 text-sm uppercase tracking-wider mb-4">Expenses Log ledger</h3>
                        ...
                        {trip.expenses.map((exp) => (
                          <tr key={exp.id} className="hover:bg-slate-50/50">
                            ...
                            <td className="py-3.5 px-2">
                              <span className="px-2 py-0.5 rounded bg-slate-100 border border-slate-200 text-[10px] font-semibold text-slate-650">
                                {exp.category}
                              </span>
                            </td>
                            <td className="py-3.5 px-2 max-w-[180px] truncate">{exp.description}</td>
                            <td className="py-3.5 px-2 text-right font-bold text-slate-900">${exp.amount.toFixed(2)}</td>
                            <td className="py-3.5 px-2 text-right">
                              <button
                                onClick={() => handleDeleteExpense(exp.id)}
                                className="p-1 rounded bg-slate-100 hover:bg-rose-500/10 border border-slate-200 text-slate-500 hover:text-rose-650 transition-all cursor-pointer"
