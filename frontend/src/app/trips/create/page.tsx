...
export default function CreateTrip() {
  ...
  if (loading || !user) {
    return (
      <div className="flex-grow flex flex-col justify-center items-center min-h-screen theme-trip-create">
  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen relative theme-trip-create">
      {/* Glow Effects */}
      ...
      {generating && (
        ...
        <div className="fixed inset-0 bg-slate-50/90 backdrop-blur-md z-50 flex flex-col justify-center items-center px-6">
          <div className="relative w-24 h-24 mb-8">
            ...
            <div className="absolute inset-4 rounded-full bg-white border border-slate-200 flex items-center justify-center">
              <Compass className="w-7 h-7 text-cyan-600 animate-pulse" />
            </div>
          </div>
          <h2 className="text-2xl font-bold text-slate-900 mb-2 text-center animate-pulse">Generating Your Custom Itinerary</h2>
          <p className="text-cyan-600 text-sm font-semibold tracking-wide uppercase">{LOADING_MESSAGES[loadingMsgIdx]}</p>
          <p className="text-slate-500 text-xs mt-8 max-w-xs text-center leading-relaxed">
      ...
      <main className="relative z-10 flex-grow max-w-3xl mx-auto w-full px-6 py-12">
        <div className="mb-8">
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2">
            Plan Your Journey <Sparkles className="w-6 h-6 text-cyan-600" />
          </h1>
          <p className="text-slate-500 text-sm mt-1">Specify destination details to invoke our AI agent.</p>
        </div>

        {error && (
          <div className="flex items-start gap-2.5 p-3.5 mb-6 rounded-lg bg-rose-500/10 border border-rose-500/20 text-rose-600 text-sm">
            <AlertCircle className="w-5 h-5 shrink-0 mt-0.5" />
            <span>{error}</span>
          </div>
        )}

        <form onSubmit={handleSubmit} className="glass-panel p-8 rounded-2xl border border-slate-200/60 shadow-xl space-y-6">
          
          {/* Destination */}
          <div>
            <label className="block text-slate-700 text-xs font-semibold uppercase tracking-wider mb-2" htmlFor="destination">
            ...
            <div className="relative">
              ...
              <input
                id="destination"
                type="text"
                className="glass-input w-full pl-11 pr-4 py-3.5 rounded-xl text-sm bg-white"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              <label className="block text-slate-700 text-xs font-semibold uppercase tracking-wider mb-2" htmlFor="startDate">
              ...
              <div className="relative">
                ...
                <input
                  id="startDate"
                  type="date"
                  className="glass-input w-full pl-11 pr-4 py-3.5 rounded-xl text-sm bg-white [color-scheme:light]"
                  value={startDate}
            ...
            <div>
              <label className="block text-slate-700 text-xs font-semibold uppercase tracking-wider mb-2" htmlFor="durationDays">
                Trip Duration (1 - 30 days) <span className="text-rose-500">*</span>
              </label>
              <div className="relative">
          ...
          <div>
            <label className="block text-slate-700 text-xs font-semibold uppercase tracking-wider mb-2">
              Budget Tier Preference <span className="text-rose-500">*</span>
            </label>
            <div className="grid grid-cols-3 gap-3">
              {(['Low', 'Medium', 'High'] as const).map((tier) => (
                <button
                  ...
                  className={`py-3.5 px-4 rounded-xl text-sm font-semibold border transition-all flex flex-col items-center justify-center gap-1.5 cursor-pointer ${
                    budgetPreference === tier
                      ? 'bg-cyan-500/10 border-cyan-500 text-cyan-600 shadow-md shadow-cyan-500/5'
                      : 'bg-white/40 border-slate-200 text-slate-500 hover:text-slate-900 hover:bg-white hover:border-slate-300'
                  }`}
                >
                  <DollarSign className={`w-4 h-4 ${budgetPreference === tier ? 'text-cyan-600' : 'text-slate-400'}`} />
                  <span>{tier}</span>
                </button>
              ))}
            </div>
          ...
          {/* Interests */}
          <div>
            <label className="block text-slate-700 text-xs font-semibold uppercase tracking-wider mb-2.5">
              ...
              {INTERESTS_OPTIONS.map((opt) => {
                ...
                return (
                  <button
                    ...
                    className={`p-3.5 rounded-xl text-xs font-medium border text-center transition-all cursor-pointer ${
                      isSelected
